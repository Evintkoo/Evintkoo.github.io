...
const CircRNAAnimations = (function() {
  ...
  function createRNAStructures() {
    ...
    structures.forEach(({ class: className, delay }) => {
      ...
      if (delay) structure.style.animationDelay = `${delay}s`;
...
// ── Shared Research Animations (runs on every research page) ──
const SharedResearchAnimations = (function () {
  'use strict';
  ...
  // Scroll-reveal for abstract items, cards, highlights, section titles
  function setupFadeIn() {
    const els = document.querySelectorAll(
      '.abstract-item, .method-card, .research-highlight, .card, .research-section__title'
    );
  ...
  // Animate numbers in hero metrics when they scroll into view
  function setupHeroCounters() {
    const els = document.querySelectorAll('.hero-metric__value, .meta-highlight__value');
    if (!els.length) return;
    ...
    function animateCounter(el) {
      ...
      const step = (now) => {
        ...
        const val = (end * easeOut(progress)).toFixed(decimals);
